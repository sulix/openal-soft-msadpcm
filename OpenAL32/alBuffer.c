...
AL_API ALvoid AL_APIENTRY alBufferData(ALuint buffer, ALenum format, const ALvoid *data, ALsizei size, ALsizei freq)
{
    ...
    al_try
    {
        ...
        switch(SrcType)
        {
            ...
            case UserFmtMSADPCM64:
                FrameSize = ChannelsFromUserFmt(SrcChannels) * 38;
                CHECK_VALUE(Context, (size%FrameSize) == 0);
                ...
                err = LoadData(ALBuf, freq, NewFormat, size/FrameSize*64,
                               SrcChannels, SrcType, data, AL_TRUE);
                if(err != AL_NO_ERROR)
                    al_throwerr(Context, err);
                break;

            case UserFmtMSADPCM128:
            ...
            case UserFmtMSADPCM256:
                FrameSize = ChannelsFromUserFmt(SrcChannels) * 134;
                CHECK_VALUE(Context, (size%FrameSize) == 0);
                ...
                err = LoadData(ALBuf, freq, NewFormat, size/FrameSize*256,
                               SrcChannels, SrcType, data, AL_TRUE);
                if(err != AL_NO_ERROR)
                    al_throwerr(Context, err);
                break;
...
AL_API ALvoid AL_APIENTRY alBufferSubDataSOFT(ALuint buffer, ALenum format, const ALvoid *data, ALsizei offset, ALsizei length)
{
    ...
    al_try
    {
        ...
        else if(ALBuf->OriginalType == UserFmtMSADPCM64)
        {
            original_align = ChannelsFromUserFmt(ALBuf->OriginalChannels) * 38;
        }
        else if(ALBuf->OriginalType == UserFmtMSADPCM128)
        {
            original_align = ChannelsFromUserFmt(ALBuf->OriginalChannels) * 70;
        }
        else if(ALBuf->OriginalType == UserFmtMSADPCM256)
        {
            original_align = ChannelsFromUserFmt(ALBuf->OriginalChannels) * 134;
        }
        else
        {
        ...
        else if(SrcType == UserFmtMSADPCM64)
        {
            offset = offset/38*64 * Bytes;
            length = length/original_align * 64;
        }
        else if(SrcType == UserFmtMSADPCM128)
        {
            offset = offset/70*128 * Bytes;
            length = length/original_align * 128;
        }
        else if(SrcType == UserFmtMSADPCM256)
        {
            offset = offset/134*256 * Bytes;
            length = length/original_align * 256;
        }
        else
        {
  ...
  ALenum channels, ALenum type, ALvoid *data)
{
    ...
    al_try
    {
        ...
        else if(type == UserFmtMSADPCM64 && (samples%64) != 0)
        {
            ReadUnlock(&ALBuf->lock);
            al_throwerr(Context, AL_INVALID_VALUE);
        }
        else if(type == UserFmtMSADPCM128 && (samples%128) != 0)
        {
            ReadUnlock(&ALBuf->lock);
            al_throwerr(Context, AL_INVALID_VALUE);
        }
        else if(type == UserFmtMSADPCM256 && (samples%256) != 0)
        {
            ReadUnlock(&ALBuf->lock);
            al_throwerr(Context, AL_INVALID_VALUE);
        }
...
typedef ALubyte ALmsadpcm64;
typedef ALubyte ALmsadpcm128;
typedef ALubyte ALmsadpcm256;
typedef struct {
    ALbyte b[3];
} ALbyte3;
...
#define CalculateMSADPCMSample(result, nibble, predictor,                     \
                               sample1, sample2, delta)                       \
{                                                                             \
    ...
    {                                                                         \
        result = (ALshort) calculatedSample;                                  \
    }                                                                         \
                                                                              \
    sample2 = sample1;                                                        \
    sample1 = result;                                                         \
...
/*
 * As all MSADPCM block sizes have an 'align' parameter of the form (2^n)-1, we
 * can unroll the loop into (p-1) iterations of q instructions where (p+q) = n, with
 * an extra (q-1) instructions left over. The optimized mono decoding routine unrolls
 * the inner decoding loop 4 times (q=4). Things are a little more complicated with
 * the stereo decoding routine as the loop is run 'align'*2 times (to account for the
 * extra channel). In this case, we still unroll the loop 4 times, but this results in
 * decoding only two samples per loop _per channel_.
 */

/* This function is an optimized version of DecodeMSADPCMBlock for Mono streams */
static void DecodeMonoMSADPCMBlock(ALshort *dst, const ALubyte *src, ALint align)
{
    ALubyte predictor;
    ALshort delta;
    ALshort samples[2];
    ...
    /* Read in the predictor */
    predictor = *(src++);

    /* Read in the delta */
    delta = *((ALshort*)(src));
    src += 2; /* An ALshort is two bytes wide */

    /* Read in the inital two samples... */
    samples[0] = *((ALshort*)(src));
    ...
    *(dst++) = samples[1];
    ...
    /* We're then going to decode four bytes per loop (8 samples),
     * so we divide 'align' (the number of times to loop) by 4.
     * We know 'align' is divisible by 4 at this point.
     */
    align = align >> 2;

    /* Read in 3 bytes (6 nibbles), calculate and output 6 samples */
    nibble[0] = *(src) >> 4;
    nibble[1] = *(src++) & 0xF;
    nibble[2] = *(src) >> 4;
    nibble[3] = *(src++) & 0xF;
    nibble[4] = *(src) >> 4;
    nibble[5] = *(src++) & 0xF;
    CalculateMSADPCMSample(*(dst+0), nibble[0], predictor,
                           samples[0], samples[1], delta);
    CalculateMSADPCMSample(*(dst+1), nibble[1], predictor,
                           samples[0], samples[1], delta);
    CalculateMSADPCMSample(*(dst+2), nibble[2], predictor,
                           samples[0], samples[1], delta);
    CalculateMSADPCMSample(*(dst+3), nibble[3], predictor,
                           samples[0], samples[1], delta);
    CalculateMSADPCMSample(*(dst+4), nibble[4], predictor,
                           samples[0], samples[1], delta);
    CalculateMSADPCMSample(*(dst+5), nibble[5], predictor,
                           samples[0], samples[1], delta);
    ...
    for(i = 0;i < align;i++)
    {
        /* Read in 4 bytes (8 nibbles), calculate and output 8 samples */
        nibble[0] = *(src) >> 4;
        nibble[1] = *(src++) & 0xF;
        nibble[2] = *(src) >> 4;
        nibble[3] = *(src++) & 0xF;
        ...
        CalculateMSADPCMSample(*(dst+5), nibble[5], predictor,
                               samples[0], samples[1], delta);
        CalculateMSADPCMSample(*(dst+6), nibble[6], predictor,
                               samples[0], samples[1], delta);
        CalculateMSADPCMSample(*(dst+7), nibble[7], predictor,
                               samples[0], samples[1], delta);

        /* Move the output pointer along 8 samples */
        dst += 8;
    }
}

/* This function is an optimized version of DecodeMSADPCMBlock for Stereo streams */
static void DecodeStereoMSADPCMBlock(ALshort *dst, const ALubyte *src, ALint align)
{
    ALubyte predictor[2];
    ALshort delta[2];
    ALshort samples[4];
    ...
    /* Read in the predictor for each channel */
    predictor[0] = *(src++);
    predictor[1] = *(src++);

    /* Read in the delta for each channel */
    delta[0] = *((ALshort*)(src));
    delta[1] = *((ALshort*)(src+2));

    /* Read in the initial samples for each channel */
    samples[0] = *((ALshort*)(src+4));
    samples[1] = *((ALshort*)(src+6));
    samples[2] = *((ALshort*)(src+8));
    samples[3] = *((ALshort*)(src+10));

    /* The deltas and intial samples take 12 bytes */
    src += 12;

    /* We are going to be decoding 3 bytes/channel here, so
     * the final loop needs to run three fewer times.
     */
    align -= 3;

    /* We decode four samples/loop (two per channel) in the
     * inner loop. 'align' originally stored the number of samples
     * per channel to decode, we now want it to store the number
     * of loop iterations, so we divide by 2.
     */
    align = align >> 1;

    /* Output the initial samples (earlier samples were read in first) */
    *(dst++) = samples[2];
    *(dst++) = samples[3];
    *(dst++) = samples[0];
    *(dst++) = samples[1];

    /* Read in 6 bytes (12 samples, 6 samples/channel) */
    nibble[0] = *(src) >> 4;
    nibble[1] = *(src++) & 0xF;
    nibble[2] = *(src) >> 4;
    nibble[3] = *(src++) & 0xF;
    ...
    /* Calculate and output 12 samples (6 per channel) */
    CalculateMSADPCMSample(*(dst+0), nibble[0], predictor[0],
                           samples[0], samples[2], delta[0]);
    CalculateMSADPCMSample(*(dst+1), nibble[1], predictor[1],
                           samples[1], samples[3], delta[1]);
    CalculateMSADPCMSample(*(dst+2), nibble[2], predictor[0],
                           samples[0], samples[2], delta[0]);
    CalculateMSADPCMSample(*(dst+3), nibble[3], predictor[1],
                           samples[1], samples[3], delta[1]);
    CalculateMSADPCMSample(*(dst+4), nibble[4], predictor[0],
                           samples[0], samples[2], delta[0]);
    CalculateMSADPCMSample(*(dst+5), nibble[5], predictor[1],
                           samples[1], samples[3], delta[1]);
    CalculateMSADPCMSample(*(dst+6), nibble[6], predictor[0],
                           samples[0], samples[2], delta[0]);
    CalculateMSADPCMSample(*(dst+7), nibble[7], predictor[1],
                           samples[1], samples[3], delta[1]);
    CalculateMSADPCMSample(*(dst+8), nibble[8], predictor[0],
                           samples[0], samples[2], delta[0]);
    CalculateMSADPCMSample(*(dst+9), nibble[9], predictor[1],
                           samples[1], samples[3], delta[1]);
    CalculateMSADPCMSample(*(dst+10), nibble[10], predictor[0],
                           samples[0], samples[2], delta[0]);
    CalculateMSADPCMSample(*(dst+11), nibble[11], predictor[1],
                           samples[1], samples[3], delta[1]);

    /* 'dst' is an ALshort pointer, so we increment it by 12 ALshort samples */
    dst += 12;

    /* Each loop here will decode 4 bytes */
    for(i = 0;i < align;i++)
    {
        /* Read 4 bytes (8 samples) */
        nibble[0] = *(src) >> 4;
        nibble[1] = *(src++) & 0xF;
        nibble[2] = *(src) >> 4;
        nibble[3] = *(src++) & 0xF;
        ...
        /* Calculate and output 8 samples */
        CalculateMSADPCMSample(*(dst+0), nibble[0], predictor[0],
                               samples[0], samples[2], delta[0]);
        CalculateMSADPCMSample(*(dst+1), nibble[1], predictor[1],
                               samples[1], samples[3], delta[1]);
...
/* "Generic" MSADPCM decoder. This is never actually used, but would be where >2 channel
 * streams would be decoded.
 */
static void DecodeMSADPCMBlock(ALshort *dst, const ALubyte *src, ALint numchans, ALint align)
{
    ...
    if(numchans == 1)
    {
        for(i = 0;i < (align * numchans);i++)
        {
            ...
            CalculateMSADPCMSample(*(dst+1), nibble[1], predictor[0],
                                   samples[0], samples[1], delta[0]);
            dst += 2;
        }
    }
...
#define DECL_TEMPLATE(T, SAMPLES, ALIGN)                                      \
static void Convert_##T##_ALmsadpcm##SAMPLES(T *dst,                          \
                                             const ALmsadpcm##SAMPLES *src,   \
                                             ALuint numchans, ALuint len)     \
{                                                                             \
    ALshort tmp[SAMPLES*MAX_INPUT_CHANNELS]; /* Max samples frame can be */   \
    ALuint i = 0;                                                             \
    ALuint j;                                                                 \
                                                                              \
    if(numchans == 1)                                                         \
    {                                                                         \
        ...
        {                                                                     \
            DecodeMonoMSADPCMBlock(tmp, src, ALIGN);                          \
            src += ALIGN+7;                                                   \
                                                                              \
            for(j = 0;j < SAMPLES && i < len;j++,i++)                         \
            {                                                                 \
    ...
    {                                                                         \
        while(i < len*2)                                                      \
        {                                                                     \
            DecodeStereoMSADPCMBlock(tmp, src, ALIGN);                        \
            src += (ALIGN+7)*2;                                               \
                                                                              \
            for(j = 0;j < SAMPLES*2 && i < len*2;j++,i++)                     \
            {                                                                 \
                *(dst++) = Conv_##T##_ALshort(tmp[j]);                        \
            }                                                                 \
        }                                                                     \
    ...
    {                                                                         \
        ...
        {                                                                     \
            ...
            src += (ALIGN+7)*numchans;                                        \
                                                                              \
            for(j = 0;j < SAMPLES*numchans && i < len*numchans;j++,i++)       \
            {                                                                 \
                *(dst++) = Conv_##T##_ALshort(tmp[j]);                        \
            }                                                                 \
        }                                                                     \
...
#define MSADPCM_DECL_TEMPLATE(T)                                              \
DECL_TEMPLATE(T, 64,  31 )                                                    \
DECL_TEMPLATE(T, 128, 63 )                                                    \
DECL_TEMPLATE(T, 256, 127)

MSADPCM_DECL_TEMPLATE(ALbyte)
MSADPCM_DECL_TEMPLATE(ALubyte)
MSADPCM_DECL_TEMPLATE(ALushort)
MSADPCM_DECL_TEMPLATE(ALint)
MSADPCM_DECL_TEMPLATE(ALuint)
MSADPCM_DECL_TEMPLATE(ALfloat)
MSADPCM_DECL_TEMPLATE(ALdouble)
MSADPCM_DECL_TEMPLATE(ALmulaw)
MSADPCM_DECL_TEMPLATE(ALalaw)
MSADPCM_DECL_TEMPLATE(ALbyte3)
MSADPCM_DECL_TEMPLATE(ALubyte3)

#undef MSADPCM_DECL_TEMPLATE

#undef DECL_TEMPLATE

/* Instead of using the templated conversion function for ALmsadpcm64->ALshort,
 * we use this version, which, instead of decoding a block into a temp buffer
 * and then converting that into the resultant format, decodes directly into
 * the final buffer. This is significantly faster, as it avoids another loop
 * over the decoded sound.
 */

#define MSADPCM_DECL_TEMPLATE(SAMPLES, ALIGN)                                 \
static void Convert_ALshort_ALmsadpcm##SAMPLES(ALshort *dst,                  \
                                               const ALmsadpcm##SAMPLES *src, \
                                               ALuint numchans, ALuint len)   \
{                                                                             \
    ALuint i = 0;                                                             \
    ...
    {                                                                         \
        while(i < len)                                                        \
        {                                                                     \
            DecodeMonoMSADPCMBlock(dst, src, ALIGN);                          \
            src += ALIGN+7;                                                   \
            dst += SAMPLES;                                                   \
            i += SAMPLES;                                                     \
        }                                                                     \
    ...
    {                                                                         \
        while(i < len*2)                                                      \
        {                                                                     \
            DecodeStereoMSADPCMBlock(dst, src, ALIGN);                        \
            src += (ALIGN+7)*2;                                               \
            dst += SAMPLES*2;                                                 \
            i += SAMPLES*2;                                                   \
        }                                                                     \
    ...
    {                                                                         \
        ...
        {                                                                     \
            ...
            src += (ALIGN+7)*numchans;                                        \
            dst += SAMPLES*numchans;                                          \
            i += SAMPLES*numchans;                                            \
        }                                                                     \
    }                                                                         \
}

MSADPCM_DECL_TEMPLATE(64,  31 )
MSADPCM_DECL_TEMPLATE(128, 63 )
MSADPCM_DECL_TEMPLATE(256, 127)

#undef MSADPCM_DECL_TEMPLATE

#define MSADPCM_DECL_TEMPLATE(SAMPLES, ALIGN)                                  \
static void Convert_ALima4_ALmsadpcm##SAMPLES(ALima4 *dst,                     \
                                              const ALmsadpcm##SAMPLES *src,   \
                                              ALuint numchans, ALuint len)     \
{                                                                              \
    /* TODO: Convert_AL_ima4_ALmsadpcm -flibit */                              \
}
...
{                                                                             \
    ...
    {                                                                         \
        ...
        case UserFmtIMA4:                                                     \
            ...
            break;                                                            \
        case UserFmtMSADPCM64:                                                \
            Convert_##T##_ALmsadpcm64(dst, src, numchans, len);               \
            break;                                                            \
        case UserFmtMSADPCM128:                                               \
            Convert_##T##_ALmsadpcm128(dst, src, numchans, len);              \
            break;                                                            \
        case UserFmtMSADPCM256:                                               \
            Convert_##T##_ALmsadpcm256(dst, src, numchans, len);              \
            break;                                                            \
        case UserFmtByte3:                                                    \
...
static void ConvertData(ALvoid *dst, enum UserFmtType dstType, const ALvoid *src, enum UserFmtType srcType, ALsizei numchans, ALsizei len)
{
    switch(dstType)
    {
        ...
        case UserFmtIMA4:
            ...
            break;
        case UserFmtMSADPCM64:
            /* TODO: MSADPCM64 encoding not supported -flibit */
            break;
        case UserFmtMSADPCM128:
            /* TODO: MSADPCM128 encoding not supported -flibit */
            break;
        case UserFmtMSADPCM256:
            /* TODO: MSADPCM256 encoding not supported -flibit */
            break;
        case UserFmtByte3:
...
static ALenum LoadData(ALbuffer *ALBuf, ALuint freq, ALenum NewFormat, ALsizei frames, enum UserFmtChannels SrcChannels, enum UserFmtType SrcType, const ALvoid *data, ALboolean storesrc)
{
    ...
    if(storesrc)
    {
        ...
        if(SrcType == UserFmtIMA4)
            ...
            ALBuf->OriginalSize = frames / 64 * 38 * ChannelsFromUserFmt(SrcChannels);
        else if(SrcType == UserFmtMSADPCM128)
            ALBuf->OriginalSize = frames / 128 * 70 * ChannelsFromUserFmt(SrcChannels);
        else if(SrcType == UserFmtMSADPCM256)
            ALBuf->OriginalSize = frames / 256 * 134 * ChannelsFromUserFmt(SrcChannels);
        else
...
ALuint BytesFromUserFmt(enum UserFmtType type)
{
    ...
    case UserFmtMSADPCM64: break; /* not handled here */
    case UserFmtMSADPCM128: break; /* not handled here */
    case UserFmtMSADPCM256: break; /* not handled here */
    }
    return 0;
}
                                     ...
                                     enum UserFmtType *type)
{
    static const struct {
        ...
        { AL_FORMAT_MONO_MSADPCM64,  UserFmtMono, UserFmtMSADPCM64  },
        { AL_FORMAT_MONO_MSADPCM128, UserFmtMono, UserFmtMSADPCM128 },
        { AL_FORMAT_MONO_MSADPCM256, UserFmtMono, UserFmtMSADPCM256 },
        ...
        { AL_FORMAT_STEREO_MSADPCM64,  UserFmtStereo, UserFmtMSADPCM64  },
        { AL_FORMAT_STEREO_MSADPCM128, UserFmtStereo, UserFmtMSADPCM128 },
        { AL_FORMAT_STEREO_MSADPCM256, UserFmtStereo, UserFmtMSADPCM256 },
